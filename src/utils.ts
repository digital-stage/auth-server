...
const sendActivationLink = (transport: Mail, email: string, code: string) => transport.sendMail({
  to: email,
...
const sendResetPasswordLink = (transport: Mail, email: string, code: string) => transport.sendMail({
  to: email,
  from: process.env.SMTP_FROM,
